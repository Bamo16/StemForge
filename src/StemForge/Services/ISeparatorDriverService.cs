...
{
    /// <summary>
    /// Raised when the driver reports its built-in preset catalog (shortly after each startup).
    /// May fire more than once if the driver process restarts after an idle timeout.
    /// </summary>
    event Action<IReadOnlyList<Preset>>? PresetsLoaded;
